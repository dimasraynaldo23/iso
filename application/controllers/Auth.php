  ...
  private function _login()
  {
  	...
  	if($user) {
  		...
  		if($user['is_active'] == 1) {
 			...
 			if(password_verify($password, $user['password'])){
  			 ...
  			 $this->session->set_flashdata('message', '<div class="alert alert-danger" role="alert">this mail has not been activated!</div>');
  			 	redirect('auth');
	...
	private function _sendEmail($token, $type)
	{
		$config = [
			...
			'smtp_user' => 'simulasiiso@gmail.com',
			'smtp_pass' => 'Edodrdp23',
			'smtp_port' => 465,
			'mailtype'  => 'html',
			'charset'   => 'utf-8',
		...
		$this->email->from('simulasiiso@gmail.com', 'Simulasi ISO');
		...
		if($user){
			...
			if($user_token){
				...
				if(time() - $user_token['date_created'] < (60*60*24)){
					...
					$this->session->set_flashdata('message', '<div class="alert alert-success" role="alert">'. $email .' has been activation! Please login.</div>');
		redirect('auth');
				} else {
